...
To support multitenant subscriptions and dynamic sponsorships, we will define/extend two collections in Payload CMS.

### Collection 1: `Sponsors` (New Collection)
Sponsors are scoped per tenant (neighborhood). This allows tenant admins to manage their own sponsors, who appear in the footer or side block of outgoing emails on free tiers.
...
* **Fields:**
  ...
  * `text` (Text, Optional) - Short description or call-to-action copy (e.g. *"Support our local hardware store!"*).
  * `weight` (Number, Default: `1`) - Used for randomized/weighted rotation if multiple sponsors are active.
  * `tenant` (Relationship to `tenants`, Required) - Scopes the sponsor to a specific neighborhood. Managed by the multi-tenant plugin.
...
* **New/Modified Fields:**
  ...
  * `unsubscribeToken` (Text, Hidden) - A unique, random secret token generated upon contact creation. Used to construct secure unsubscribe links, avoiding passing raw IDs or emails in URLs.
...
A base React component wraps all broadcasts. It will conditionally inject the sponsor banner and dynamic unsubscribe links:
...
3. **Sponsor Section (Conditional):** If the tenant is on the free tier and has active sponsors, the renderer fetches a random/weighted sponsor and injects:
   * A "Sponsored by [Name]" text.
   * An linked image banner.
   * The sponsor's short description text.
4. **Footer Section:** 
